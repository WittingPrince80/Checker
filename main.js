...
function gotResult(error,results)
{
    ...
    drawn_sketch = results[0].label;
      ...
      document.getElementById('gconfidence').innerHTML = 'Guess Confidence: ' + Math.round(results[0].confidence * 100) + '%';

    if(drawn_sketch==itemToDraw)
    {
            co = Math.round(drawn_sketch.confidence * 100)
    document.getElementById('confidence').innerHTML = 'Confidence: ' + co + '%';
         ff = co/10;
         sf = 10 - (timer/2);
    pscore = ff + sf;
    if(pscore > score)
...
{
    timer_counter++;
    timer = Math.floor(timer_counter / 100)
